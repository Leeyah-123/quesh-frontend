...
export function ProfilePlan({ user }: ProfilePlanProps) {
  ...
  const plans = [
    {
      ...
      price: "$0",
      period: "forever",
      features: ["5 documents per month", "100 questions per month", "50MB storage limit", "Standard response time"],
  ...
  const handleUpgrade = (planName: string) => {
    ...
    setTimeout(() => {
      ...
      alert(`Upgrade to ${planName} plan initiated. This would redirect to payment in a real app.`)
    }, 1500)
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <TiltCard
              ...
              intensity={10}
              scale={1.02}
              className={`h-full ${plan.popular ? "ring-2 ring-purple-500/50" : ""}`}
            >
              <Card
                className={`glass border-slate-700 h-full flex flex-col ${plan.isCurrent ? "bg-slate-800/50" : ""}`}
              >
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="flex items-center">
                        {plan.icon && <span className="mr-2 text-purple-400">{plan.icon}</span>}
                        {plan.name}
                      </CardTitle>
                      <CardDescription>{plan.description}</CardDescription>
                    </div>
                    {plan.popular && <Badge className="bg-purple-600 hover:bg-purple-700">Popular</Badge>}
                  </div>
                </CardHeader>
