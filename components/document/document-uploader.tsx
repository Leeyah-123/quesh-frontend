...
import { Upload, LinkIcon, File, X, Check, AlertCircle, Loader2 } from "lucide-react"
...
export function DocumentUploader() {
  ...
  const handleLinkUpload = async () => {
    ...
    try {
      ...
      const documentType = linkPreview?.type.toLowerCase().includes("pdf") ? "pdf" : "docx"
              ...
              <TabsContent value="link" className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="flex gap-2">
                    ...
                    <Button
                      onClick={linkPreview?.isValid ? handleLinkUpload : validateAndUploadLink}
                  ...
                  {linkPreview && (
                    <TiltCard intensity={8} scale={1.02} className="glass p-4 rounded-lg">
                      <div className="flex items-center">
                        <div className="bg-slate-800 p-2 rounded mr-3">
                          <File className="h-4 w-4 text-purple-400" />
                        </div>
                        <div className="flex-1">
                          <p className="text-sm font-medium truncate">{linkPreview.title}</p>
                          <p className="text-xs text-gray-400">{linkPreview.type}</p>
                        </div>
                      </div>
                    </TiltCard>
                  )}

                  <p className="text-xs text-gray-400 text-center">Supported formats: PDF, DOCX. Direct links only.</p>
                </div>
              </TabsContent>
            </Tabs>
          </motion.div>
            ...
            <div className="flex items-center mb-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">
                  {activeTab === "file" ? fileName : linkPreview?.title || "Document"}
                </p>
                {activeTab === "file" && (
                  <p className="text-xs text-gray-400">
                    {(fileSize / 1024 / 1024).toFixed(2)} MB • {fileType === "application/pdf" ? "PDF" : "DOCX"}
                  </p>
                )}
              </div>
              ...
              <div className="relative">
                ...
                {uploadState === "uploading" && uploadProgress < 100 && (
                  <div className="absolute top-0 left-0 h-full w-full overflow-hidden">
                    <div className="h-full w-1/3 bg-white/10 animate-shimmer" style={{ backgroundSize: "200% 100%" }} />
                  </div>
                )}
              </div>
