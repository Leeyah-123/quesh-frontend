...
export function DocumentChat({ documentId }: DocumentChatProps) {
  ...
  // Get current session (cached with useMemo)
  const currentSession = useMemo(() => getCurrentSession(), [documentId]);
  ...
  const getAIResponse = (query: string): string => {
    ...
    if (
      ...
      query.toLowerCase().includes('findings') ||
      query.toLowerCase().includes('results')
    ) {
      ...
      query.toLowerCase().includes('method') ||
    ...
    <>
      <div className="flex h-screen bg-gradient-to-b from-slate-950 to-slate-900 text-white">
        ...
        <div className="w-64 border-r border-slate-800 glass flex flex-col">
          ...
          <div className="p-4 border-b border-slate-800">
            <div
              className="flex items-center mb-3 cursor-pointer hover:bg-purple-500/20 hover:border-purple-500/50 rounded-sm py-1"
              role="button"
              title="View Document"
              onClick={() => {
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  ...
                  {currentSession.documentSize
                    ? ` ${(currentSession.documentSize / 1024 / 1024).toFixed(
                        2
                      )} MB`
                    : ' From URL'}
                </p>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="border-b border-slate-800 glass p-4 flex items-center">
            ...
            <div className="ml-auto text-sm text-gray-400">
              {currentSession.questionCount} questions asked
            </div>
          </div>
      ...
      {showDocumentViewer && (
        <DocumentViewer
          documentName={currentSession.documentTitle}
